...
static void gatcherChildDetailByReverse(Json::Value &detail, WrapperTraceNode &head)
{
  ...
  if (!childrenDetail.empty()) { detail.append(childrenDetail); }
}
...
Json::Value mergeTraceNodeTree(WrapperTraceNode &root) { return mergeChildren(root); }
...
{
  std::call_once(_pool_init_flag, []() { _con_pool.initPool(global_agent_info.co_host); });
