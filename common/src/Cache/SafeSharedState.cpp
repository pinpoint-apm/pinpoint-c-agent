...
bool SafeSharedState::checkTraceLimit(int64_t timestamp)
{
  ...
  if (global_agent_info.trace_limit == -1) { return false; }
  ...
  return false;
BLOCK:
  pp_trace("This span dropped. max_trace_limit:%ld current_tick:%lld onLine:%d", global_agent_info.trace_limit, this->_global_state->tick,
    this->isReady() ? (1) : (0));
  return true;
}
