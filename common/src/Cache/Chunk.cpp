...
{
  if (ready_cks.empty()) { return length; }
...
{
  if (this->free_cks.empty()) { return length; }
...
{
  while (this->c_resident_size < this->ck_alloc_size && !this->free_cks.empty()) { this->reduceFreeCK(); }
}


uint32_t Chunks::getAllocSize() const { return this->ck_alloc_size; }

Chunks::Chunks(uint32_t max_size, uint32_t resident_size) : c_resident_size(resident_size), c_max_size(max_size), threshold(1024)
{
  this->ck_alloc_size = 0;
  this->ck_free_ck_capacity = 0;
  if (max_size < resident_size) throw std::invalid_argument("chunks: max_size must bigger then resident_size");
}
...
bool Chunks::useExistingChunk(uint32_t length) const
{
  ...
  if (this->ck_free_ck_capacity >= length) { return true; }
...
int Chunks::copyDataIntoChunks(const void* data, uint32_t length)
{
  ...
  // 2. free ck
  // 3. create a new ck
  if ((ret = copyDataIntoReadyCK(p_in, length)) == 0) { goto DONE; }

  p_in += (length - ret);
  length = ret;

  if ((ret = copyDataIntoFreeCK(p_in, length)) == 0) { goto DONE; }
