...
PPAgentT global_agent_info = {"unix:./pinpoint_test.sock", 1, 10, 1500, 1, nullptr, nullptr, nullptr};
...
NodeID pinpoint_get_per_thread_id() { return __tls_id; }
...
static NodeID do_start_trace(NodeID id, const char *opt = nullptr, va_list *args = nullptr)
{
  if (id <= E_INVALID_NODE) {
    ...
    // pass opt
    if (opt != nullptr) { trace->setOpt(opt, args); }
...
static void flush_to_agent(std::string &span)
{
  ...
  if (!trans->copy_into_send_buffer(span)) { pp_trace("drop current span as it's too heavy! size:%lu", span.length()); }
...
NodeID pinpoint_end_trace(NodeID traceId)
{
  try {
    ...
    if (ret == E_ROOT_NODE) { freeNodeTree(traceId); }
...
uint64_t pinpoint_start_time(void) { return (uint64_t) SafeSharedState::instance().getStartTime(); }

int64_t generate_unique_id() { return SafeSharedState::instance().generateUniqueId(); }

void reset_unique_id(void) { return SafeSharedState::instance().resetUniqueId(); }
...
int check_tracelimit(int64_t timestamp) { return SafeSharedState::instance().checkTraceLimit(timestamp) ? (1) : (0); }
...
{
  if (likely(handler != nullptr)) { _SpanHandler_ = std::bind(handler, std::placeholders::_1); }
}
...
void show_status(void)
{
  ...
  auto add_alive_node_fun = [&status](TraceNode &node) { status["pool_alive_nodes"].append(node.mPoolIndex); };
