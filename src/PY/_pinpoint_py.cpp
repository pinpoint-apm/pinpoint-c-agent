...
static void _get_read_lock(void) { pthread_rwlock_rdlock(&rwlock); }

static void _release_lock(void) { pthread_rwlock_unlock(&rwlock); }

static void _get_write_lock(void) { pthread_rwlock_wrlock(&rwlock); }
...
void _free_common_shared(void) { pthread_rwlock_destroy(&rwlock); }
#else
...
bool set_collector_host(const char* host) {
...
static PyObject* py_set_agent(PyObject* self, PyObject* args, PyObject* keywds) {
  ...
  char* kwlist[] = {default_host, default_tracelimit, NULL};
...
static PyMethodDef PinpointMethods[] = {
    ...
    {"trace_has_root", py_trace_has_root, METH_VARARGS,
     "def trace_has_root(int id=-1)-> long # check current whether have a root. \n True: \nFalse: "
    ...
    {"set_context_key", py_pinpoint_context_key, METH_VARARGS,
     "def set_context_key(string key,string value,int id=-1): # create a key-value pair that bases "
     "on current trace chain"},
    {"get_context_key", py_pinpoint_get_key, METH_VARARGS,
     "def get_context_key(string key,int id=-1)->string "},
    {"check_tracelimit", py_check_tracelimit, METH_VARARGS,
     "def check_tracelimit(long timestamp=-1): #check trace whether is limit"},
    {"enable_debug", py_pinpoint_enable_utest, METH_VARARGS,
     "def enable_debug(callback):#enable logging output(callback )"},
    {"force_flush_trace", py_force_flush_span, METH_VARARGS,
     "def force_flush_trace(timeout=3,int id=-1): #force flush span during timeout"},
    {"mark_as_error", py_pinpoint_mark_an_error, METH_VARARGS,
     "def mark_as_error(string msg,string file_name,uint line_no,int id=-1): #Found an error in "
     "this trace"},
    {"set_agent", (PyCFunction)py_set_agent, METH_VARARGS | METH_KEYWORDS,
     ...
     "tcp:host:port\",trace_limit=-1): # set pinpint collector information"},
    {NULL, NULL, 0, NULL}};
