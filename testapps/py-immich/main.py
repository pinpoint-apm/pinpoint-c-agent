# Copy from https://github.com/immich-app/immich/blob/main/machine-learning/app/main.py
# use as an example for add pinpointPy
...
from pinpointPy import set_agent, monkey_patch_for_pinpoint, enable_experiment_plugins, use_asyncio_local_context
import logging

set_agent("cd.dev.test.immich", "cd.dev.test.py",
...
# spools to disk if payload is 64 MiB or larger
MultiPartParser.max_file_size = 2**26
...
class UserMiddleWare(PinPointMiddleWare):
    async def dispatch(self, request, call_next):
        if request.url.path in ["/heartbeat", "/l7check", "/ping"]:
    ...
    try:
        if settings.request_threads > 0:
            ...
            thread_pool = ThreadPoolExecutor(
                settings.request_threads) if settings.request_threads > 0 else None
            log.info(
...
async def load(model: InferenceModel) -> InferenceModel:
    ...
    def _load(model: InferenceModel) -> InferenceModel:
        if model.load_attempts > 1:
            raise HTTPException(
                500, f"Failed to load model '{model.model_name}'")
        with lock:
            try:
                model.load()
