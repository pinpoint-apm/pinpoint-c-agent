...
class AsynPinTrace(object):
    ...
    def onBefore(self,*args, **kwargs):
        id = pinpointId.get()
        new_id = _pinpointPy.start_trace(id)
        pinpointId.set(new_id)
        self.node_id = new_id
        return (args,kwargs)

    def onEnd(self,ret):
        new_id = _pinpointPy.end_trace(self.node_id)
        pinpointId.set(new_id)
    ...
    def generateTid(self):
        return ('%s^%s^%s') % (APP_ID,str(_pinpointPy.start_time()), str(_pinpointPy.unique_id()))
